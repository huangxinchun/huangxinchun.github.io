...
简单工厂模式是类的创建模式，又叫做`静态工厂方法`（Static Factory Method）模式。简单工厂模式是由一个工厂对象决定创建出哪一种产品类的实例。
...
> 总结: 1.依赖表现为局部变量,函数参数,或调用静态方法。2.关联通常表现为全局变量。关联分根据语义区别可以分为聚合跟组合。聚合表现为全局变量的对象数组中的一个。组合也是全局变量。一般在构造方法中要实例化~
